...
//			System.out.println(scores[3]);	// 배열에 없는 4번째 값을 불러오려고 하니 ArrayIndexOutOfBoundsException 예외가 발생한다.
		...
		} catch (ArithmeticException e) {	// 예외 처리할 코드를 catch문 안에 넣어주고, 어떤 예외가 발생할 때 catch문을 실행시킬 지 괄호 안에 넣어준다. ArithmeticException을 변수 e에 담아준다.
			System.out.println("계산이 잘못되었습니다." + e.getMessage());	// ArithmeticException이 발생할 경우, 실행할 코드와 e.getMessage()를 통해 예외 원인도 출력
			e.printStackTrace();	// printStackTrcae()를 통해 예외 발생도 출력
